...
namespace Yarn.Compiler.Upgrader
{
    ...
    public struct TypeBinding
    {
        ...
        public static List<TypeBinding> UnifyBindings(IEnumerable<TypeBinding> typeBindings)
        {
            ...
            IEnumerable<IGrouping<string, TypeBinding>> variableBindingTypeGroups = typeBindings.GroupBy(b => b.VariableName, b => b);

            List<TypeBinding> unifiedVariableBindings = variableBindingTypeGroups.Select(bindingGroup =>
            {
                // If there is precisely one type binding, keep it, if it's
                // defined
                IEnumerable<TypeBinding> definedBindings = bindingGroup.Where(b => b.Type != BuiltinTypes.Undefined);
