namespace Yarn.Compiler
{
    ...
    class LastLineBeforeOptionsVisitor : YarnSpinnerParserBaseVisitor<byte>
    {
        ...
        {
            YarnSpinnerParser.HashtagContext hashtag = new YarnSpinnerParser.HashtagContext(context, 0);
        ...
        // chunks its way through the if, any else-ifs and elses public block of statements
        public override byte VisitIf_statement([NotNull] YarnSpinnerParser.If_statementContext context)
        {
            ...
            YarnSpinnerParser.Else_if_clauseContext[] elseifs = context.else_if_clause();
            if (elseifs?.Length > 0)
            {
                foreach(YarnSpinnerParser.Else_if_clauseContext elif in elseifs)
                {
                    RunThroughStatements(elif.statement());
                }
            }

            YarnSpinnerParser.StatementContext[] el = context.else_clause()?.statement();
            if (el?.Length > 0)
            {
        ...
        public override byte VisitShortcut_option_statement([NotNull] YarnSpinnerParser.Shortcut_option_statementContext context)
        {
            ...
            {
                YarnSpinnerParser.StatementContext[] statements = shortcut.statement();
        ...
        private void RunThroughStatements(YarnSpinnerParser.StatementContext[] statements)
        {
            for (int i = 0; i < statements.Length; i++)
            {
                ...
                YarnSpinnerParser.Line_statementContext previous = statements[i - 1].line_statement();
                if (previous == null)
                {
                    continue;
                }
