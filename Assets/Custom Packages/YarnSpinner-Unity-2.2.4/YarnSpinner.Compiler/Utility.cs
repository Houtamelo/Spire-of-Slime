namespace Yarn.Compiler
{
    ...
    public static class Utility
    {
        ...
        public static string AddTagsToLines(string contents, ICollection<string> existingLineTags = null)
        {
            ...
            (FileParseResult parseSource, IEnumerable<Diagnostic> diagnostics) = ParseSource(contents);
            ...
            // each new line tag.
            UntaggedLineListener untaggedLineListener = new UntaggedLineListener(new List<string>(existingLineTags), parseSource.Tokens);
        ...
        public static (FileParseResult, IEnumerable<Diagnostic>) ParseSource(string source)
        {
            ...
            FileParseResult result = Compiler.ParseSyntaxTree("<input>", source, ref diagnostics);
        ...
        {   
            ...
            public override void ExitLine_statement([NotNull] YarnSpinnerParser.Line_statementContext context)
            {
                ...
                string[] texts = StringTableGeneratorVisitor.GetHashtagTexts(hashtags);
                ...
                int previousTokenIndex = IndexOfPreviousTokenOnChannel(
                                                                       TokenStream,
                ...
                if (previousTokenIndex == -1)
                {
                    ...
                    // public error - there must be at least one symbol
                ...
                IToken previousToken = TokenStream.Get(previousTokenIndex);

                // Generate a new, unique line ID.
                string newLineID = GenerateString(existingStrings);
        ...
        public static List<List<string>> ExtractStringBlocks(IEnumerable<Node> nodes)
        {
            ...
            {
                IEnumerable<BasicBlock> blocks = InstructionCollectionExtensions.GetBasicBlocks(node);
            ...
            void RunBlock(BasicBlock block, IEnumerable<BasicBlock> blocks, HashSet<string> visited, string openingLineID = null)
            {
                ...
                List<string> runOfLines = new List<string>();
                ...
                foreach (BasicBlock.PlayerVisibleContentElement content in block.PlayerVisibleContent)
                {
                    ...
                    if (content is BasicBlock.LineElement)
                    {
                        ...
                        BasicBlock.LineElement line = content as BasicBlock.LineElement;
                    ...
                    else if (content is BasicBlock.OptionsElement)
                    {
                        ...
                        BasicBlock.OptionsElement options = content as BasicBlock.OptionsElement;
                        Dictionary<string, BasicBlock> jumpOptions = new Dictionary<string, BasicBlock>();
                        foreach (BasicBlock.OptionsElement.Option option in options.Options)
                        {
                            BasicBlock destination = blocks.First(b => b.LabelName == option.Destination);
                        ...
                        foreach (KeyValuePair<string, BasicBlock> pair in jumpOptions)
                        {
        ...
        public static List<List<GraphingNode>> DetermineNodeConnections(string[] YarnFileContents)
        {
            ...
            foreach (string contents in YarnFileContents)
            {
                (FileParseResult parseSource, IEnumerable<Diagnostic> diagnostics) = ParseSource(contents);

                List<GraphingNode> connections = new List<GraphingNode>();
                JumpGraphListener jumpListener = new JumpGraphListener(connections);
    ...
    public static class InstructionCollectionExtensions
    {
        ...
        public static IEnumerable<BasicBlock> GetBasicBlocks(this Node node)
        {
            ...
            HashSet<int> leaderIndices = new HashSet<int>
                                         {
                                             // The first instruction is a leader.
                                             0,
                                         };

            foreach (KeyValuePair<string, int> label in node.Labels)
            {
            ...
            // block.
            List<Instruction> currentBlockInstructions = new List<Instruction>();
            ...
            BasicBlock GetBlock(string label)
            {
                ...
                {
                    // nothing found
                    throw new InvalidOperationException($"No block in {node.Name} starts at index {index}");
                }
            }
            ...
            foreach (BasicBlock block in result)
            {
                List<string> optionDestinations = new List<string>();
                ...
                foreach (Instruction instruction in block.Instructions)
                {
                    switch (instruction.Opcode)
                    {
                        case Instruction.Types.OpCode.AddOption:
                            {
                                ...
                                // to a node.
                                Operand destinationNodeName = instruction.Operands.ElementAt(1);
                        ...
                        case Instruction.Types.OpCode.Jump:
                            {
                                ...
                                foreach (string destinationLabel in optionDestinations)
                                {
                                    BasicBlock destinationBlock = GetBlock(destinationLabel);
                        ...
                        case Instruction.Types.OpCode.JumpTo:
                            {
                                BasicBlock destinationBlock = GetBlock(instruction.Operands.ElementAt(0).StringValue);
                        ...
                        case Instruction.Types.OpCode.JumpIfFalse:
                            {
                                string destinationLabel = instruction.Operands.ElementAt(0).StringValue;

                                BasicBlock destinationFalseBlock = GetBlock(destinationLabel);
                                BasicBlock destinationTrueBlock = GetBlockWithIndex(block.FirstInstructionIndex + count + 1);
                ...
                if (block.Destinations.Count() == 0)
                {
                    ...
                    {
                        int nextBlockStartInstruction = block.FirstInstructionIndex + block.Instructions.Count();

                        BasicBlock destination = GetBlockWithIndex(nextBlockStartInstruction);
    ...
    public class BasicBlock
    {
        ...
        public IEnumerable<BasicBlock> Descendants
        {
            get 
            {
                ...
                {
                    BasicBlock next = candidates.Dequeue();
                    ...
                    foreach (BasicBlock destination in next.Destinations.Where(d => d.Block != null).Select(d=> d.Block))
                    {
        ...
        public IEnumerable<PlayerVisibleContentElement> PlayerVisibleContent
        {
            get
            {
                List<(string LineID, string Destination)> accumulatedOptions = new List<(string LineID, string Destination)>();
