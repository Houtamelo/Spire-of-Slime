...
namespace UI
{
	[RequireComponent(requiredComponent: typeof(CanvasRenderer)), AddComponentMenu(menuName: "UI/Sliced Filled Image", order: 11)]
	public class SlicedFilledImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
	{
		private static class SetPropertyUtility
		{
			...
			{
				if( EqualityComparer<T>.Default.Equals( x: currentValue, y: newValue ) )
					return false;
			...
			{
				if( ( currentValue == null && newValue == null ) || ( currentValue != null && currentValue.Equals( obj: newValue ) ) )
					return false;
...
#pragma warning restore 1692
		...
		{
			base.OnValidate();
			m_PixelsPerUnitMultiplier = Mathf.Max( a: 0.01f, b: m_PixelsPerUnitMultiplier );
		}
#endif
		...
		protected override void OnPopulateMesh( VertexHelper vh )
		{
			if( activeSprite == null )
			{
				base.OnPopulateMesh( vh: vh );
				return;
			}

			GenerateSlicedFilledSprite( vh: vh );
		}
		...
		protected override void UpdateMaterial()
		{
			...
			if( activeSprite == null )
			{
				canvasRenderer.SetAlphaTexture( texture: null );
				return;
			}
		...
		private void GenerateSlicedFilledSprite( [NotNull] VertexHelper vh )
		{
			...
			Vector4 outer = Sprites.DataUtility.GetOuterUV( sprite: activeSprite );
			Vector4 padding = Sprites.DataUtility.GetPadding( sprite: activeSprite );

			if( !hasBorder )
			{
				Vector2 size = activeSprite.rect.size;

				int spriteW = Mathf.RoundToInt( f: size.x );
				int spriteH = Mathf.RoundToInt( f: size.y );
			...
			Vector4 inner = Sprites.DataUtility.GetInnerUV( sprite: activeSprite );
			Vector4 border = GetAdjustedBorders( border: activeSprite.border / pixelsPerUnit, adjustedRect: rect );

			padding = padding / pixelsPerUnit;

			s_SlicedVertices[0] = new Vector2( x: padding.x,              y: padding.y );
			s_SlicedVertices[3] = new Vector2( x: rect.width - padding.z, y: rect.height - padding.w );
			...
			s_SlicedUVs[0] = new Vector2( x: outer.x, y: outer.y );
			s_SlicedUVs[1] = new Vector2( x: inner.x, y: inner.y );
			s_SlicedUVs[2] = new Vector2( x: inner.z, y: inner.w );
			s_SlicedUVs[3] = new Vector2( x: outer.z, y: outer.w );
			...
			for( int x = 0; x < 3; x++ )
			{
				...
				for( int y = 0; y < 3; y++ )
				{
					...
					Vector4 vertices = new( x: s_SlicedVertices[x].x, y: s_SlicedVertices[y].y, z: s_SlicedVertices[x2].x, w: s_SlicedVertices[y2].y );
					Vector4 uvs = new( x: s_SlicedUVs[x].x,           y: s_SlicedUVs[y].y,      z: s_SlicedUVs[x2].x,      w: s_SlicedUVs[y2].y );
					float fillAmount = ( m_FillAmount - sliceStart ) / ( sliceEnd - sliceStart );

					GenerateFilledSprite( vh: vh, vertices: vertices, uvs: uvs, fillAmount: fillAmount );
				}
			}
		}
		...
		private Vector4 GetAdjustedBorders( Vector4 border, Rect adjustedRect )
		{
			...
			for( int axis = 0; axis <= 1; axis++ )
			{
				...
				if( originalRect.size[index: axis] != 0 )
				{
					borderScaleRatio = adjustedRect.size[index: axis] / originalRect.size[index: axis];
					border[index: axis] *= borderScaleRatio;
					border[index: axis + 2] *= borderScaleRatio;
				}

				// If the rect is smaller than the combined borders, then there's not room for the borders at their normal size.
				// In order to avoid artefacts with overlapping borders, we scale the borders down to fit.
				float combinedBorders = border[index: axis] + border[index: axis + 2];
				if( adjustedRect.size[index: axis] < combinedBorders && combinedBorders != 0 )
				{
					borderScaleRatio = adjustedRect.size[index: axis] / combinedBorders;
					border[index: axis] *= borderScaleRatio;
					border[index: axis + 2] *= borderScaleRatio;
				}
			}
		...
		private void GenerateFilledSprite( VertexHelper vh, Vector4 vertices, Vector4 uvs, float fillAmount )
		{
			...
			s_Vertices[0] = new Vector3( x: vertices.x, y: vertices.y );
			s_Vertices[1] = new Vector3( x: vertices.x, y: vertices.w );
			s_Vertices[2] = new Vector3( x: vertices.z, y: vertices.w );
			s_Vertices[3] = new Vector3( x: vertices.z, y: vertices.y );

			s_UVs[0] = new Vector2( x: uvLeft,  y: uvBottom );
			s_UVs[1] = new Vector2( x: uvLeft,  y: uvTop );
			s_UVs[2] = new Vector2( x: uvRight, y: uvTop );
			s_UVs[3] = new Vector2( x: uvRight, y: uvBottom );

			int startIndex = vh.currentVertCount;

			for( int i = 0; i < 4; i++ )
				vh.AddVert( position: s_Vertices[i], color: color, uv0: s_UVs[i] );

			vh.AddTriangle( idx0: startIndex,     idx1: startIndex + 1, idx2: startIndex + 2 );
		...
		float ILayoutElement.preferredWidth
		{
			get
			{
				...
				return Sprites.DataUtility.GetMinSize( sprite: activeSprite ).x / pixelsPerUnit;
			}
		}
		...
		float ILayoutElement.preferredHeight
		{
			get
			{
				...
				return Sprites.DataUtility.GetMinSize( sprite: activeSprite ).y / pixelsPerUnit;
			}
		}
		...
		bool ICanvasRaycastFilter.IsRaycastLocationValid( Vector2 screenPoint, Camera eventCamera )
		{
			...
			Vector2 local;
			if( !RectTransformUtility.ScreenPointToLocalPointInRectangle( rect: rectTransform, screenPoint: screenPoint, cam: eventCamera, localPoint: out local ) )
				return false;
			...
			Vector4 adjustedBorder = GetAdjustedBorders( border: border / pixelsPerUnit, adjustedRect: rect );

			for( int i = 0; i < 2; i++ )
			{
				if( local[index: i] <= adjustedBorder[index: i] )
					continue;

				if( rect.size[index: i] - local[index: i] <= adjustedBorder[index: i + 2] )
				{
					local[index: i] -= ( rect.size[index: i] - spriteRect.size[index: i] );
					continue;
				}

				float lerp = Mathf.InverseLerp( a: adjustedBorder[index: i], b: rect.size[index: i] - adjustedBorder[index: i + 2], value: local[index: i] );
				local[index: i] = Mathf.Lerp( a: border[index: i], b: spriteRect.size[index: i] - border[index: i + 2], t: lerp );
			}

			// Normalize local coordinates.
			Rect textureRect = activeSprite.textureRect;
			Vector2 normalized = new( x: local.x / textureRect.width, y: local.y / textureRect.height );

			// Convert to texture space.
			float x = Mathf.Lerp( a: textureRect.x, b: textureRect.xMax, t: normalized.x ) / activeSprite.texture.width;
			float y = Mathf.Lerp( a: textureRect.y, b: textureRect.yMax, t: normalized.y ) / activeSprite.texture.height;
			...
			try
			{
				return activeSprite.texture.GetPixelBilinear( u: x, v: y ).a >= alphaHitTestMinimumThreshold;
			}
			catch( UnityException e )
			{
				Debug.LogError( message:
				               $"Using alphaHitTestMinimumThreshold greater than 0 on Image whose sprite texture cannot be read. {e} Also make sure to disable sprite packing for this sprite.", context: this );
				return true;
			}
		}
		...
		{
			m_FillAmount = Mathf.Clamp01( value: m_FillAmount );
		}
		...
		private static void RebuildImage( SpriteAtlas spriteAtlas )
		{
			...
			{
				SlicedFilledImage image = m_TrackedTexturelessImages[index: i];
				if( spriteAtlas.CanBindTo( sprite: image.activeSprite ) )
				{
					image.SetAllDirty();
					m_TrackedTexturelessImages.RemoveAt( index: i );
				}
			}
		}
#endif
