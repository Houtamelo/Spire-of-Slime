...
namespace Yarn.Unity.Editor
{
    ...
    public class YarnProjectImporter : ScriptedImporter, IYarnErrorSource
    {
        ...
        {
            public static List<Yarn.IType> BuiltInTypesList = new List<Yarn.IType> {
        ...
        public override void OnImportAsset(AssetImportContext ctx)
        {
            ...
            localDeclarations = result.Declarations.ToArray();
            
            foreach (Diagnostic diagnostic in result.Diagnostics)
            {
                if (diagnostic.Severity is not Diagnostic.DiagnosticSeverity.Error)
                {
                    string message = $"{ctx.assetPath}: {diagnostic}";
                    string type = diagnostic.Severity is Diagnostic.DiagnosticSeverity.Warning ? "Warning:" : "Info:";
                    ctx.LogImportWarning($"{type} {message}");
                }
            }

            Diagnostic[] errors = result.Diagnostics.Where(d => d.Severity == Diagnostic.DiagnosticSeverity.Error).ToArray();
            ...
            localDeclarations = localDeclarations
                .Where(decl => decl.Name.StartsWith("$Yarn.Internal") == false).ToArray();
            ...
            var scriptImporters = sourceScripts.Where(s => s != null).Select(s => AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(s)) as YarnImporter).ToArray();

            // First step: check to see if there's any parse errors in the
            // files.
            var scriptsWithParseErrors = scriptImporters.Where(script => script.isSuccessfullyParsed == false).ToArray();

            if (scriptsWithParseErrors.Length != 0)
            {
            ...
            errors = compilationResult.Diagnostics.Where(d => d.Severity == Diagnostic.DiagnosticSeverity.Error).ToArray();
            ...
            if (errors.Any()) 
            {
                ...
                {
                    var errorMessages = errorGroup.Select(e => e.ToString()).ToArray();
        ...
        {
            var pathsToImporters = sourceScripts.Where(s => s != null).Select(s => AssetDatabase.GetAssetPath(s)).ToArray();

            if (pathsToImporters.Length == 0)
            {
        ...
        public IEnumerable<LineMetadataTableEntry> GenerateLineMetadataEntries()
        {
            CompilationResult? compilationResult = CompileStringsOnly();
    ...
    public class ReorderableDeclarationsList
    {
        ...
        private void OnAdd(ReorderableList list)
        {
            ...
            typeProp.stringValue = BuiltinTypes.String.Name;
