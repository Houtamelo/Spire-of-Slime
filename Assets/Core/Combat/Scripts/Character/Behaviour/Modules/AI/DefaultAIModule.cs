...
{
    public record DefaultAIRecord : AIRecord
    {
        [NotNull]
        public override IAIModule Deserialize(CharacterStateMachine owner) => new DefaultAIModule(owner);
    ...
    public class DefaultAIModule : IAIModule
    {
        ...
        [NotNull]
        public AIRecord GetRecord() => new DefaultAIRecord();
        ...
        /// <summary> Do not use unless confirmed that _owner.Display.IsSome </summary>
        ...
        public void Heuristic()
        {
            ...
            IndexableHashSet<CharacterStateMachine> allies = CombatManager.Characters.GetEditable(_owner.PositionHandler.IsLeftSide);
            IndexableHashSet<CharacterStateMachine> enemies = CombatManager.Characters.GetEditable(_owner.PositionHandler.IsLeftSide == false);
            ...
            CustomValuePooledList<string> logs = new(capacity: 16);

            ReadOnlySpan<ISkill> skills = Script.Skills;
            ...
            for (int i = 0; i < skills.Length; i++)
            {
                ...
                IndexableHashSet<CharacterStateMachine> possibleTargets = skill.IsPositive ? allies : enemies;
                for (int j = 0; j < possibleTargets.Count; j++)
                {
                    ...
                    {
                        logs.Add($"{skill.DisplayName} cannot be cast on {target.Script.CharacterName} due to computed points being: {computedPoints}, allow allies: {skill.IsPositive}");
                    }
                ...
                float pointsSum = 0;
                foreach ((_, _, float points) in lease)
                    pointsSum += points;
            ...
            else
            {
                ...
                if (LogHeuristics)
                    Log(ref logs);
#endif
                RecoveryModule.SetInitial(TSpan.FromSeconds(1.0));
            }
...
#if UNITY_EDITOR
            void Log(ref CustomValuePooledList<string> refLogs)
            {
        ...
        private int CheckMarksAndEnsureTargetChanceLimit([NotNull] ISkill skill, [NotNull] Lease<(CharacterStateMachine target, float rawPoints, float points)> source)
        {
            if (skill.MultiTarget || skill.IsPositive)
                return RemoveNegativesAndZeroes(source);

            using CustomValuePooledList<int> charactersWithMark = new(capacity: 4);
            ...
            if (markedCount == 0)
            {
                ...
                totalPoints = 0;
                foreach ((_, _, float points) in source)
                    totalPoints += points;
        ...
        /// <returns> Adjusted length of source lease. </returns>
        private static int RemoveNegativesAndZeroes([NotNull] Lease<(CharacterStateMachine target, float rawPoints, float points)> source)
        {
            for (int i = 0; i < source.Length; i++)
            {
                (CharacterStateMachine _, float _, float points) = source.Rented[i];
        ...
        private static (ISkill skill, CharacterStateMachine target) GetWeightedRandom([NotNull] Dictionary<ISkill, Lease<(CharacterStateMachine, float, float)>> source)
        {
            float pointsSum = 0f;
