...
namespace Yarn.Compiler.Upgrader
{
    ...
    public struct UpgradeResult
    {
        ...
        public static UpgradeResult Merge(UpgradeResult a, UpgradeResult b)
        {
            ...
            IEnumerable<OutputFile> onlyResultA = a.Files.Where(f1 => b.Files.Select(f2 => f2.Path).Contains(f1.Path) == false);
            IEnumerable<OutputFile> onlyResultB = b.Files.Where(f1 => a.Files.Select(f2 => f2.Path).Contains(f1.Path) == false);

            IEnumerable<OutputFile> mergedFiles = filePairs.Select(pair => OutputFile.Merge(pair.FileA, pair.FileB));

            IEnumerable<OutputFile> allFiles = onlyResultA.Concat(onlyResultB).Concat(mergedFiles);
        ...
        public struct OutputFile
        {
            ...
            public static OutputFile Merge(OutputFile a, OutputFile b)
            {
                if (a.Path != b.Path)
                {
                ...
                IOrderedEnumerable<TextReplacement> mergedReplacements = a.Replacements
    ...
    public static class LanguageUpgrader
    {
        ...
        /// text.</throws>
        public static string ApplyReplacements(string originalText, IEnumerable<TextReplacement> replacements)
        {
            ...
            IOrderedEnumerable<TextReplacement> sortedList = replacements.OrderBy((r) => r.Start);
            ...
            foreach (TextReplacement replacement in sortedList)
            {
                ...
                // method)
                string existingSubstring = text.ToString(replacement.Start + offset, replacement.OriginalText.Length);
                ...
                int lengthDifference = replacement.ReplacementText.Length - replacement.OriginalText.Length;
