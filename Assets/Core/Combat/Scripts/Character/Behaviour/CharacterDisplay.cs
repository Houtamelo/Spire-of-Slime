...
namespace Core.Combat.Scripts.Behaviour
{
    ...
    public class CharacterDisplay : MonoBehaviour, IEquatable<CharacterDisplay>
    {
        ...
        public void SetCombatManager(CombatManager manager, TimelineIcon timelineIcon)
        {
            Debug.Assert(timelineIcon != null);
        ...
        public Option<Vector3> GetCuePosition()
        {
            if (AnimationStatus is AnimationStatus.Grappled)
            {
                ...
                foreach (StatusInstance status in owner.StatusModule.GetAll)
                    if (status is LustGrappled lustGrappled && lustGrappled.Restrainer.Display.TrySome(out CharacterDisplay restrainerDisplay))
    ...
    #region StatusIcons
        ...
        public void CreateIconForStatus(StatusInstance statusInstance)
        {
        ...
        private void ClearStatusIconAndReassign()
        {
            ...
            foreach (StatusInstance status in StateMachine.Value.StatusModule.GetAll)
            {
        ...
        public void StatusIconRemoved(StatusInstance effectInstance)
        {
    ...
    #region Bars
        ...
        public void SetStaminaBar(bool active, uint current, uint max) => staminaBar.Set(active, current, max);

        [SerializeField, Required]
        private LustBar lustBar;
        public void SetLustBar(bool active, uint lust) => lustBar.Set(active, lust);

        [SerializeField, Required]
        private TemptationBar temptationBar;
        public void SetTemptationBar(bool active, float temptation) => temptationBar.Set(active, temptation);
        ...
        public void SetRecoveryBar(bool active, float remaining, float total) => actionBars.SetRecovery(active, remaining, total, CombatManager);
        
        public void SetDownedBar(bool active, float remaining, float total) => actionBars.SetDowned(active, remaining, total, CombatManager);
        
        public void SetChargeBar(bool active, float remaining, float total) => actionBars.SetCharge(active, remaining, total, CombatManager);
        
        public void SetStunBar(bool active, float remaining, float total) => actionBars.SetStun(active, remaining, total, CombatManager);
    ...
    #region Timeline Icon
        private TimelineIcon _timelineIcon; // provided by combat manager

        public void SetTimelineCuePosition(float time, string text, Color color)
        {
            _timelineIcon.SetLabel(text, color);
            _timelineIcon.SetTime(time, CombatManager);
        }

        public void AllowTimelineIcon(bool value) => _timelineIcon.gameObject.SetActive(value);
    #endregion
    ...
    #region Prediction Icon
        ...
        public void SetStateMachine(CharacterStateMachine stateMachine)
        {
            ...
            _timelineIcon.SetSprite(script.TimelineIcon);
        ...
        public void MatchAnimationWithState(CharacterState state)
        {
            ...
            switch (state)
            {
                ...
                case CharacterState.Grappling:
                    gameObject.SetActive(true);
                    foreach (StatusInstance status in owner.StatusModule.GetAllRelated)
                    {
        ...
        public void AnimateDefeated(Action<CharacterDisplay> onFinish)
        {
            Option<CharacterStateMachine> stateMachine = StateMachine;
        ...
        public void DeAnimateGrappledFromStunnedRestrainer(CharacterStateMachine restrainer)
        {
            ...
            if (restrainer.Display.TrySome(out CharacterDisplay restrainerDisplay) == false || _renderer.TrySome(out ICharacterRenderer selfRenderer) == false)
            {
                DeAnimateGrappled(restrainer);
                return;
            }
        ...
        public bool Equals(CharacterDisplay other) => other == this;
