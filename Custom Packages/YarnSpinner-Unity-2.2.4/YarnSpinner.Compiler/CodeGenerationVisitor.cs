namespace Yarn.Compiler
{
    ...
    // Instructions
    public partial class CodeGenerationVisitor : YarnSpinnerParserBaseVisitor<int>
    {
        ...
        private int GenerateCodeForExpressionsInFormattedText(IList<IParseTree> nodes)
        {
            ...
            foreach (IParseTree child in nodes)
            {
        ...
        public override int VisitLine_statement(YarnSpinnerParser.Line_statementContext context)
        {
            ...
            // stack.
            int expressionCount = this.GenerateCodeForExpressionsInFormattedText(context.line_formatted_text().children);

            // Get the lineID for this string from the hashtags
            YarnSpinnerParser.HashtagContext lineIDTag = Compiler.GetLineIDTag(context.hashtag());
        ...
        public override int VisitCommand_statement(YarnSpinnerParser.Command_statementContext context)
        {
            ...
            foreach (IParseTree node in context.command_formatted_text().children)
            {
        ...
        private void GenerateCodeForFunctionCall(string functionName, YarnSpinnerParser.Function_callContext functionContext, YarnSpinnerParser.ExpressionContext[] parameters)
        {
            ...
            foreach (YarnSpinnerParser.ExpressionContext parameter in parameters)
            {
        ...
        public override int VisitIf_statement(YarnSpinnerParser.If_statementContext context)
        {
            ...
            // handle the if
            YarnSpinnerParser.If_clauseContext ifClause = context.if_clause();
            this.GenerateClause(endOfIfStatementLabel, ifClause, ifClause.statement(), ifClause.expression());

            // all elseifs
            foreach (YarnSpinnerParser.Else_if_clauseContext elseIfClause in context.else_if_clause())
            {
            ...
            YarnSpinnerParser.Else_clauseContext elseClause = context.else_clause();
            if (elseClause != null)
            {
        ...
        private void GenerateClause(string jumpLabel, ParserRuleContext clauseContext, YarnSpinnerParser.StatementContext[] children, YarnSpinnerParser.ExpressionContext expression)
        {
            ...
            foreach (YarnSpinnerParser.StatementContext child in children)
            {
                this.Visit(child);
            }
        ...
        public override int VisitShortcut_option_statement(YarnSpinnerParser.Shortcut_option_statementContext context)
        {
            ...
            // For each option, create an public destination label that, if
            ...
            // options.
            foreach (YarnSpinnerParser.Shortcut_optionContext shortcut in context.shortcut_option())
            {
                // Generate the name of public label that we'll jump to if
                ...
                int expressionCount = this.GenerateCodeForExpressionsInFormattedText(shortcut.line_statement().line_formatted_text().children);

                // Get the line ID from the hashtags if it has one
                YarnSpinnerParser.HashtagContext lineIDTag = Compiler.GetLineIDTag(shortcut.line_statement().hashtag());
                string lineID = lineIDTag.text.Text;
            ...
            foreach (YarnSpinnerParser.Shortcut_optionContext shortcut in context.shortcut_option())
            {
                ...
                // option.
                foreach (YarnSpinnerParser.StatementContext child in shortcut.statement())
                {
                    this.Visit(child);
                }
        ...
        private void GenerateCodeForOperation(Operator op, IToken operatorToken, IType type, params ParserRuleContext[] operands)
        {
            // Generate code for each of the operands, so that their value is
            // now on the stack.
            foreach (ParserRuleContext operand in operands)
            {
                this.Visit(operand);
            }
        ...
        public static readonly Dictionary<int, Operator> TokensToOperators = new Dictionary<int, Operator>
        {
