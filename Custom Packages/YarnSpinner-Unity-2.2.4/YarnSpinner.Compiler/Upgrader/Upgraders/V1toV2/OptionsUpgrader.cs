...
namespace Yarn.Compiler.Upgrader
{
    ...
    public class OptionsUpgrader : ILanguageUpgrader
    {
        public UpgradeResult Upgrade(UpgradeJob upgradeJob)
        {
            ...
            foreach (CompilationJob.File file in upgradeJob.Files)
            {
                ...
                OptionsVisitor formatFunctionVisitor = new OptionsVisitor(replacements);
        ...
        private class OptionsVisitor : YarnSpinnerV1ParserBaseVisitor<int>
        {
            ...
            public override int VisitNode([NotNull] YarnSpinnerV1Parser.NodeContext context)
            {
                ...
                List<string> newShortcutOptionEntries = new List<string>();

                bool optionsWereRenamed = false;

                foreach (OptionLink optionLink in this.currentNodeOptionLinks)
                {
                    ...
                    bool needsNewline = optionLink.Context.hashtag().Length > 0;
                    ...
                    string optionLine = GetContextTextWithWhitespace(optionLink.Context.option_formatted_text());
                    string optionDestination = optionLink.Context.NodeName?.Text ?? "<ERROR: invalid destination>";
                    ...
                    IEnumerable<string> hashtags = optionLink.Context.hashtag().Select(hashtag => GetContextTextWithWhitespace(hashtag));

                    IEnumerable<string> conditions = optionLink.Conditions.Select(c =>
                    {
                    ...
                    string allConditions = string.Join(" && ", conditions);
                ...
                IToken endOfNode = context.BODY_END().Symbol;
                ...
                TextReplacement newOptionsReplacement = new TextReplacement
                                                        {
            ...
            {
                string destination = context.NodeName.Text;

                bool nodesWereRenamed = false;
                if (destination.Contains(".")) {
                    destination = destination.Replace(".", "_");
                    nodesWereRenamed = true;
                }

                string comment = JumpSyntaxWasUpgradedDescription + (nodesWereRenamed ? " " + OptionDestinationsWereRenamedDescription : string.Empty);
            ...
            {
                OptionLink link = new OptionLink(context);
                ...
                while (parent != null && parent is YarnSpinnerV1Parser.NodeContext == false)
                {
                    ...
                    else if (parent is YarnSpinnerV1Parser.Else_if_clauseContext elseIfContext)
                    {
                        ...
                        YarnSpinnerV1Parser.If_statementContext parentIfClause = elseIfContext.Parent as YarnSpinnerV1Parser.If_statementContext;

                        foreach (IParseTree siblingClause in parentIfClause.children)
                        {
                    ...
                    else if (parent is YarnSpinnerV1Parser.Else_clauseContext elseContext)
                    {
                        ...
                        // statement must evaluate to false, in order for
                        // this option to run.
                        YarnSpinnerV1Parser.If_statementContext parentIfClause = elseContext.Parent as YarnSpinnerV1Parser.If_statementContext;
            ...
            public override int VisitHeader([NotNull] YarnSpinnerV1Parser.HeaderContext context)
            {
                ...
                string nodeName = context.header_value.Text;

                if (nodeName.Contains(".")) {
                    string newNodeName = nodeName.Replace(".", "_");
