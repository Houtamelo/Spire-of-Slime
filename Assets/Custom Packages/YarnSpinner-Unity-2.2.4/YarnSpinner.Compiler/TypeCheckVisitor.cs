namespace Yarn.Compiler
{
    ...
    /// </summary>
    public class TypeCheckVisitor : YarnSpinnerParserBaseVisitor<IType>
    {
        // {0} = variable name
        ...
        protected override IType DefaultResult => null;

        public IEnumerable<Diagnostic> Diagnostics => diagnostics;

        public override IType VisitNode(YarnSpinnerParser.NodeContext context)
        {
            ...
            YarnSpinnerParser.BodyContext body = context.body();
        ...
        public override IType VisitValueNull([NotNull] YarnSpinnerParser.ValueNullContext context)
        {
        ...
        public override IType VisitValueString(YarnSpinnerParser.ValueStringContext context)
        {
            return BuiltinTypes.String;
        }

        public override IType VisitValueTrue(YarnSpinnerParser.ValueTrueContext context)
        {
            return BuiltinTypes.Boolean;
        }

        public override IType VisitValueFalse(YarnSpinnerParser.ValueFalseContext context)
        {
            return BuiltinTypes.Boolean;
        }

        public override IType VisitValueNumber(YarnSpinnerParser.ValueNumberContext context)
        {
            return BuiltinTypes.Number;
        }

        public override IType VisitValueVar(YarnSpinnerParser.ValueVarContext context)
        {

            return VisitVariable(context.variable());
        }

        public override IType VisitVariable(YarnSpinnerParser.VariableContext context)
        {
            // The type of the value depends on the declared type of the
            // variable

            string name = context.VAR_ID()?.GetText();
            ...
            foreach (DeferredTypeDiagnostic hmm in deferredTypes)
            {
                if (hmm.Name == name)
                {
                    return BuiltinTypes.Undefined;
            ...
            Diagnostic diagnostic = new Diagnostic(sourceFileName, context, string.Format(CantDetermineVariableTypeError, name));
        ...
        public override IType VisitValueFunc(YarnSpinnerParser.ValueFuncContext context)
        {
            ...
            if (functionDeclaration == null)
            {
                ...
                List<IType> parameterTypes = context.function_call().expression()
            ...
            {
                FunctionType a = (FunctionType)functionDeclaration.Type;
            ...
            YarnSpinnerParser.ExpressionContext[] suppliedParameters = context.function_call().expression();

            List<IType> expectedParameters = functionType.Parameters;

            if (suppliedParameters.Length != expectedParameters.Count())
            {
                // Wrong number of parameters supplied
                string parameters = expectedParameters.Count() == 1 ? "parameter" : "parameters";
            ...
            {
                YarnSpinnerParser.ExpressionContext suppliedParameter = suppliedParameters[i];

                IType expectedType = expectedParameters[i];

                IType suppliedType = this.Visit(suppliedParameter);
        ...
        public override IType VisitExpValue(YarnSpinnerParser.ExpValueContext context)
        {
        ...
        public override IType VisitExpParens(YarnSpinnerParser.ExpParensContext context)
        {
        ...
        public override IType VisitExpAndOrXor(YarnSpinnerParser.ExpAndOrXorContext context)
        {
            IType type = CheckOperation(context, context.expression(), CodeGenerationVisitor.TokensToOperators[context.op.Type], context.op.Text);
            context.Type = type;
            return type;
        }

        public override IType VisitSet_statement([NotNull] YarnSpinnerParser.Set_statementContext context)
        {
            ...
            IType variableType = base.Visit(variableContext);
            ...
            IType expressionType = base.Visit(expressionContext);
            ...
            switch (context.op.Type)
            {
                case YarnSpinnerLexer.OPERATOR_ASSIGNMENT:
                    ...
                    else if (variableType == BuiltinTypes.Undefined && expressionType != BuiltinTypes.Undefined)
                    {
                        ...
                        bool canCreateDefaultValue = TryGetDefaultValueForType(expressionType, out IConvertible defaultValue);
        ...
        private IType CheckOperation(ParserRuleContext context, ParserRuleContext[] terms, Operator operationType, string operationDescription, params IType[] permittedTypes)
        {
            List<IType> termTypes = new List<IType>();

            IType expressionType = BuiltinTypes.Undefined;

            foreach (ParserRuleContext expression in terms)
            {
            ...
            if (expressionType == BuiltinTypes.Undefined)
            {
                ...
                // it's that type.
                IEnumerable<IType> typesImplementingMethod = types
            ...
            foreach (ParserRuleContext term in terms)
            {
                if (term is YarnSpinnerParser.ExpValueContext)
                {
                    YarnSpinnerParser.ValueContext value = ((YarnSpinnerParser.ExpValueContext)term).value();
                    if (value is YarnSpinnerParser.ValueFuncContext)
                    {
                        string id = ((YarnSpinnerParser.ValueFuncContext)value).function_call().FUNC_ID().GetText();

                        Declaration functionDeclaration = NewDeclarations.Where(d => d.Type is FunctionType).FirstOrDefault(d => d.Name == id);
                        if (functionDeclaration != null)
                        {
                            FunctionType func = functionDeclaration.Type as FunctionType;
            ...
            IEnumerable<YarnSpinnerParser.VariableContext> variableContexts = terms
            ...
            IEnumerable<YarnSpinnerParser.VariableContext> undefinedVariableContexts = variableContexts
            ...
            if (undefinedVariableContexts.Count() > 0)
            {
                ...
                foreach (YarnSpinnerParser.VariableContext undefinedVariableContext in undefinedVariableContexts)
                {
                    ...
                    bool canGetDefaultValue = TryGetDefaultValueForType(expressionType, out IConvertible defaultValue);
            ...
            if (termTypes.All(t => t == expressionType) == false)
            {
                ...
                // type.
                string typeList = string.Join(", ", termTypes.Select(t => t.Name));
            ...
            if (operationType != Operator.None)
            {
                ...
                IType implementingType = TypeUtil.FindImplementingTypeForMethod(expressionType, operationType.ToString());
            ...
            if (permittedTypes.Count() > 0)
            {
                ...
                else
                {
                    ...
                    string permittedTypesList = string.Join(" or ", permittedTypes.Select(t => t?.Name ?? "undefined"));
                    string typeList = string.Join(", ", termTypes.Select(t => t.Name));
            ...
            else
            {
                ...
                IType implementingTypeForMethod = TypeUtil.FindImplementingTypeForMethod(expressionType, operationType.ToString());
        ...
        private static bool TryGetDefaultValueForType(IType expressionType, out IConvertible defaultValue)
        {
        ...
        public override IType VisitIf_clause(YarnSpinnerParser.If_clauseContext context)
        {
            VisitChildren(context);
            // If clauses are required to be boolean
            YarnSpinnerParser.ExpressionContext[] expressions = new[] { context.expression() };
            return CheckOperation(context, expressions, Operator.None, "if statement", BuiltinTypes.Boolean);
        }

        public override IType VisitElse_if_clause(YarnSpinnerParser.Else_if_clauseContext context)
        {
            VisitChildren(context);
        ...
        public override IType VisitExpAddSub(YarnSpinnerParser.ExpAddSubContext context)
        {

            YarnSpinnerParser.ExpressionContext[] expressions = context.expression();

            IType type;

            Operator @operator = CodeGenerationVisitor.TokensToOperators[context.op.Type];
        ...
        public override IType VisitExpMultDivMod(YarnSpinnerParser.ExpMultDivModContext context)
        {
            YarnSpinnerParser.ExpressionContext[] expressions = context.expression();
            ...
            IType type = CheckOperation(context, expressions, @operator, context.op.Text);
            context.Type = type;
            return type;
        }

        public override IType VisitExpComparison(YarnSpinnerParser.ExpComparisonContext context)
        {
            ParserRuleContext[] terms = context.expression();

            Operator @operator = CodeGenerationVisitor.TokensToOperators[context.op.Type];

            IType type = CheckOperation(context, terms, @operator, context.op.Text);
            context.Type = type;
        ...
        public override IType VisitExpEquality(YarnSpinnerParser.ExpEqualityContext context)
        {
            ParserRuleContext[] terms = context.expression();
            ...
            // same type
            IType determinedType = CheckOperation(context, terms, @operator, context.op.Text);
        ...
        public override IType VisitExpNegative(YarnSpinnerParser.ExpNegativeContext context)
        {
        ...
        public override IType VisitExpNot(YarnSpinnerParser.ExpNotContext context)
        {
            ParserRuleContext[] terms = new[] { context.expression() };
